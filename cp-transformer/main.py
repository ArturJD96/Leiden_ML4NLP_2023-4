...
#     29                               
# )
info_load_model = None if MODE == 'train' else ('exp', 23) #None
path_gendir = pathlib.Path('gen_midis/')
num_songs = 50 # [AJD] MUST CHANGE TO CURRENT SIZE OF THE VALIDATION SET !!!
...
def write_midi(words, path_outfile, word2event):
    ...
    for i in range(len(words)):
        ...
        if vals[3] == 'Metrical':
            ...
            if vals[2] == 'Bar':
                bar_cnt += 1
            elif isinstance(vals[2], Iterable) and 'Beat' in vals[2]: #[AJD] added check for vals[2] iterability.
                beat_pos = int(vals[2].split('_')[1])
                cur_pos = int(bar_cnt * BAR_RESOL + beat_pos * TICK_RESOL) #[AJD] added int to current position
...
class TransformerModel(nn.Module):
    ...
    def inference_from_scratch(self, dictionary):
        ...
        with torch.no_grad():
            ...
            print('------ generate ------')
            while(cnt_bar < 100): #[AJD] was True
                # sample others
...
def train():
    ...
    if info_load_model:
        ...
        path_ckpt = pathlib.Path(info_load_model[0]) # path to ckpt dir
        loss = info_load_model[1] # loss
        name = 'loss_' + str(loss)
        # path_saved_ckpt = os.path.join(path_ckpt, name + '_params.pt')
        path_saved_ckpt = path_ckpt / f'{name}_params.pt'
...
def generate():
    ...
    while sidx < num_songs:
        ...
        path_outfile = path_gendir / f'get_{sidx}.mid'
